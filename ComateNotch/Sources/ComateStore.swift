...
final class ComateStore: ObservableObject {
    ...
    func start() {
        ...
        cloudTimer = Timer.scheduledTimer(withTimeInterval: 30.0, repeats: true) { [weak self] _ in
            ...
            // 复用这个 30 秒节拍做更新轮询，实际请求由 6 小时节流控制
            self.checkForUpdate()
        }
    ...
    /// 网络失败静默：更新提醒是增值信息，不影响任务状态等主功能。
    func checkForUpdate(force: Bool = false) {
        let now = Date()
        if !force,
           let last = UserDefaults.standard.object(forKey: ComateStore.lastUpdateCheckKey) as? Date,
           now.timeIntervalSince(last) < ComateStore.updateCheckInterval {
            return
        }
